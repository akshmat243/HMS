...
class HotelReview(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            base = f"{self.hotel.name}-{self.user.id}-{uuid.uuid4().hex[:6]}"
...
class RestaurantReview(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            base = f"{self.menu_item.name if self.menu_item else 'menu'}-{self.user.id}-{uuid.uuid4().hex[:6]}"
            self.slug = slugify(base)
        super().save(*args, **kwargs)
...
class ServiceReview(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            base = f"{self.service_type}-{self.user.id}-{uuid.uuid4().hex[:6]}"
            self.slug = slugify(base)
        super().save(*args, **kwargs)
